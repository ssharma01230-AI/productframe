...
export default async function StudioPage({ searchParams }: { searchParams: Promise<{ message?: string; job_id?: string; image_count?: string; view?: string; step?: string; product?: string | string[] }> }) {
  const { message, job_id: jobId, image_count: imageCount, view, step, product } = await searchParams;
  ...
  return <>
    <StudioHome workspace={workspace} userId={userId} apiError={apiError} message={message} activeRunId={jobId} products={products} initialCreate={view === 'create'} initialSelectedIds={selectedProductIds} initialOutputStep={view === 'create' && step === 'outputs'} />
