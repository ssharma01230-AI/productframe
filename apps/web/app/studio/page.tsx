...
export default async function StudioPage({ searchParams }: { searchParams: Promise<{ message?: string; job_id?: string; image_count?: string }> }) {
  const { message, job_id: jobId, image_count: imageCount } = await searchParams;
  const { userId, getToken } = await auth();
  const token = await getToken();
  let workspace: { name: string; role: string } | null = null;
  let results: AnalysisResults | null = null;
  let apiError = false;
  let products: { id:string; name:string; image_url:string|null }[] = [];
  if (token) {
    try {
  ...
  return <>
    <StudioHome workspace={workspace} userId={userId} apiError={apiError} message={message} activeRunId={jobId} products={products} />
