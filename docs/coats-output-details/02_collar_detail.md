# Coats — Collar and Lapel Detail

- Global category: `outerwear`
- Eligible family: `coats` only
- Channel: `ecommerce`
- Document key: `02_collar_detail` (documentation identifier, not a registered runtime template ID)
- Presentation mode: `invisible_mannequin`
- Benchmark: [View image](references/02_collar_detail.png)
...
Supplied 02; visible inner facing and blank label-like tab are benchmark observations, not features to add.
...
- Visible construction: Actual neckline, lapels, front edge, nearby buttons and chest pocket only where present.
- Garment volume: Natural collar roll and edge thickness, softly shaped upper chest.
...
- Required visual coverage for faithful reproduction: Sharp upper-front reference; neckline interior source if exposed.
